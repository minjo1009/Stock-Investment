...
for (const [tabId, file] of Object.entries(tabFiles)) {
  ...
  if (tabId === "home") {
    ...
    expect(source.includes("height: 430"), "portfolio holdings summary card must use fixed readable card height");
    expect(source.includes("readableRowsScroller"), "portfolio must use a readable vertical row list instead of clipped wide table");
    expect(source.includes("PortfolioHoldingSummaryRow"), "portfolio must render diagnostic holdings as readable mobile rows");
    expect(source.includes('flexBasis: "47%"'), "portfolio row metrics must wrap into readable two-column chips");
    expect(source.includes("종목별 진단 요약"), "portfolio table header must describe the readable diagnostic summary");
    expect(source.includes('flexBasis: "48%"'), "portfolio backtest KPI cards must use two-column mobile layout");
    expect(source.includes('region: "진단"'), "portfolio diagnostic row subtitle must stay compact");
