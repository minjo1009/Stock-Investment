import type { HomeReadModel } from "./common";

// Scaffold-only wrapper derived from src/mocks/fixtures/home.json.
// NOT_AUTHORITY: not backend truth, broker truth, source truth, trading permission,
// deployment readiness, or real-capital permission.
export const homeFixture = {
  generatedAt: "2026-06-22T00:00:00Z",
  contractVersion: "frontend-read-model-v1",
  readPath: "json_catalog",
  governance: {
    strategyAcceptance: "NOT_ACCEPTED",
    deploymentReadiness: "DIAGNOSTIC_ONLY_NOT_DEPLOYMENT_READY",
    realCapital: "FORBIDDEN",
    brokerMutationPermitted: false,
    paperPermission: false,
    livePermission: false,
    killSwitchActive: true,
    controlStateSource: "docs/operating_system/project_operating_state.md",
    authorityReportPath:
      "docs/reports/task_frontend_read_model_fixtures_domain_contracts/task_frontend_read_model_fixtures_domain_contracts.md",
  },
  sourceSummary: {
    freshCount: 1,
    staleCount: 1,
    missingCount: 1,
    unknownCount: 1,
    strictGateOpenCount: 0,
  },
  blockers: [
    {
      blockerId: "home-source-stale",
      severity: "P1",
      label: "출처 최신화 미완료",
      reason: "임시 화면 데이터에는 stale, missing, unknown 상태가 포함됩니다.",
      sourceRefs: ["docs/frontend_app_ssot/08_FRONTEND_READ_MODEL_CONTRACT.md"],
      detectedAt: "2026-06-22T00:00:00Z",
    },
  ],
  disabledActions: [
    {
      actionId: "execute",
      label: "거래 작업 비활성화",
      actionState: "disabled",
      disabledReason: "현재 운영 상태에서는 거래 변경 작업을 차단합니다.",
      requiredGovernanceChange: [
        "strategyAcceptance must change from NOT_ACCEPTED",
        "deploymentReadiness must change from DIAGNOSTIC_ONLY_NOT_DEPLOYMENT_READY",
        "realCapital must change from FORBIDDEN",
      ],
    },
  ],
  relativeReturnChart: {
    chartId: "home-relative-return-vs-qqq",
    title: "수익현황",
    benchmarkSymbol: "QQQ",
    selectedResolution: "1D",
    allowedResolutions: ["1D", "1H", "30M", "15M", "5M"],
    chartState: {
      chartId: "home-relative-return-vs-qqq",
      status: "SOURCE_NOT_ATTACHED",
      sourceIds: [],
      blockerReason:
        "실계좌 평가금, 원금, QQQ 점별 벤치마크는 아직 연결되지 않았습니다. 홈 차트는 선택된 진단 백테스트 스냅샷만 읽습니다.",
    },
    points: [],
    sourceState: {
      sourceId: "home-performance-timeline-source",
      sourceLabel: "평가금/원금/QQQ 성과 차트 출처",
      freshnessStatus: "MISSING",
      observedAt: null,
      generatedAt: null,
      sourceCount: null,
      strictGateAllowed: false,
      proxyAllowed: false,
      provenanceRefs: ["docs/frontend_app_ssot/08_FRONTEND_READ_MODEL_CONTRACT.md"],
      blockerReason:
        "실계좌 평가금, 원금, QQQ 점별 벤치마크는 아직 연결되지 않았습니다.",
    },
  },
  portfolioSnapshot: {
    accountValue: null,
    cash: null,
    investedCash: null,
    openPnl: null,
    realizedPnl: null,
    totalReturnPct: null,
    winRatePct: null,
    maxDrawdownPct: null,
    sourceState: {
      sourceId: "fixture-portfolio-source",
      sourceLabel: "포트폴리오 임시 출처",
      freshnessStatus: "UNKNOWN",
      observedAt: null,
      generatedAt: "2026-06-22T00:00:00Z",
      sourceCount: null,
      strictGateAllowed: false,
      proxyAllowed: false,
      provenanceRefs: ["docs/frontend_app_ssot/08_FRONTEND_READ_MODEL_CONTRACT.md"],
      blockerReason: "포트폴리오 값은 임시 화면 데이터이며 브로커 저장소가 아닙니다.",
    },
  },
  brainSnapshot: {
    candidateCount: 2,
    blockedCount: 1,
    reviewOnlyCount: 1,
    latestRuntimeDecisionAt: null,
    sourceState: {
      sourceId: "fixture-brain-source",
      sourceLabel: "브레인 임시 출처",
      freshnessStatus: "STALE",
      observedAt: "2026-06-01T00:00:00Z",
      generatedAt: "2026-06-22T00:00:00Z",
      sourceCount: 2,
      strictGateAllowed: false,
      proxyAllowed: false,
      provenanceRefs: ["docs/frontend_app_ssot/08_FRONTEND_READ_MODEL_CONTRACT.md"],
      blockerReason: "브레인 출처는 임시 화면 데이터에서 stale 상태입니다.",
    },
  },
  attentionQueue: [
    {
      itemId: "attention-source-missing",
      kind: "source",
      label: "필수 출처 연결 필요",
      reason: "필수 출처가 없으므로 판단 항목은 보류 상태로 둡니다.",
      severity: "P1",
      route: "/system",
      sourceRefs: ["docs/frontend_app_ssot/08_FRONTEND_READ_MODEL_CONTRACT.md"],
    },
  ],
  freshnessSummary: [
    {
      sourceId: "fixture-source-fresh",
      sourceLabel: "정상 임시 출처",
      freshnessStatus: "FRESH",
      observedAt: "2026-06-22T00:00:00Z",
      generatedAt: "2026-06-22T00:00:00Z",
      sourceCount: 1,
      strictGateAllowed: false,
      proxyAllowed: false,
      provenanceRefs: ["docs/frontend_app_ssot/08_FRONTEND_READ_MODEL_CONTRACT.md"],
      blockerReason: null,
    },
    {
      sourceId: "fixture-source-missing",
      sourceLabel: "누락 임시 출처",
      freshnessStatus: "MISSING",
      observedAt: null,
      generatedAt: null,
      sourceCount: null,
      strictGateAllowed: false,
      proxyAllowed: false,
      provenanceRefs: ["docs/frontend_app_ssot/08_FRONTEND_READ_MODEL_CONTRACT.md"],
      blockerReason: "필수 출처가 없습니다.",
    },
  ],
  blockerSummary: [
    {
      blockerId: "home-fixture-not-authority",
      severity: "P0",
      label: "임시 데이터는 권위가 아님",
      reason: "생성된 JSON 임시 데이터는 화면 스캐폴드 전용이며 거래를 승인할 수 없습니다.",
      sourceRefs: ["docs/frontend_app_ssot/08_FRONTEND_READ_MODEL_CONTRACT.md"],
      detectedAt: "2026-06-22T00:00:00Z",
    },
  ],
} satisfies HomeReadModel;
