...
## Task3917 L1-L5 Institutional Hardening Package

Task3917 adds diagnostic-only pure package utilities for the institutional
hardening roadmap after the Task3914 contract pass.

Current classification:

| File | Bucket | Meaning |
| --- | --- | --- |
| `src/brain/institutional_hardening/*.py` | `canonical_package_validation_candidate` | Pure diagnostic scorers and validators for failure learning, alpha decay, regime fit, crowding/liquidity, portfolio contribution, exit score, risk clamp, and OOS plan validation. |
| `src/validation/l1_l5_institutional_hardening_validator.py` | `canonical_package_validation_candidate` | Integrated package validator for the diagnostic hardening surface. |

Promotion boundary:

- These modules may compute bounded diagnostic scores and blocked states from
  supplied inputs only.
- They may not run replay, read prices, compute PnL, mutate selector/sizing,
  create order intent, permit paper/live orders, mutate broker/runtime state, or
  change acceptance/deployment/real-capital status.
