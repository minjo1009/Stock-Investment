...
- latest GPT-Codex loop correction is Task3815: Task3811 is superseded by user clarification, and the loop ledger now formalizes row schema, status lifecycle, next-loop queue semantics, and the rule that ranked candidates are not implementation authorization. Product screens remain blocked.
