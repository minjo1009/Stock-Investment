You are reviewing a local, uncommitted trading-system repository state for Codex.

Do not rely on GitHub as current state. Recent L0-L3 work is local and uncommitted. Use the context packet below as current project-state evidence.
...
Layer 3 still looks very early. The current relation graph count is only 27, which feels too low given the amount of L0 raw/source data. Review whether current L3 relation graph design is too coarse or too narrow, and how to concretely improve L3 relation graphs.

Your task:

Use the context packet below to review Layer 3's goal, graph taxonomy, graph keys, data expansion path, and validators. Do not suggest making trading signals, orders, rankings, sizing, paper/live, or broker paths.
