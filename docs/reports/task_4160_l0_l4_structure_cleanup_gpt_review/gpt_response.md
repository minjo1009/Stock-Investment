...
* **Codex는 코드 리팩터링 없이** `active_layer_handoff.md`와 `active_layer_manifest.json`만 만들고, L0 백필/컨트롤러/기존 validator는 건드리지 않는 것이 맞습니다.
...
**결론:**
TASK-4160은 **“구조 리팩터링”이 아니라 “활성 정본 인덱스 생성” 작업**으로 해야 합니다.
...
**주의:**
L0 active files는 **수정 대상이 아니라 읽기/검증 대상**입니다. TASK-4160에서 L0 코드를 바꾸면 안 됩니다.
...
| Manifest 처리  | `active_entrypoints`에 Codex가 로컬에서 확인한 실제 L1 script/validator 경로만 기입 |
...
**권장:**
이번 TASK-4160에서는 **정확히 2개 파일만 생성**하는 것이 가장 안전합니다.
검증 로그가 꼭 필요하면 `validation_log.md`까지 3개로 제한합니다.

---

# 6. `active_layer_handoff.md`에 들어가야 할 내용
...
`python -m json.tool`로 JSON 문법만 확인하면 충분합니다.
