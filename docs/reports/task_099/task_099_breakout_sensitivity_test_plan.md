...
python -m json.tool docs/reports/task_099/task_099_breakout_sensitivity_results.json > nul
```

Current-doc validation (available now):

```bash
python -m json.tool docs/reports/task_099/task_099_breakout_sensitivity_test_plan.json > nul
```

## 11) Output Schema (Expected for T099 execution result)
- File: `docs/reports/task_099/task_099_breakout_sensitivity_results.json`
- Required top-level keys:
  - `task`, `status`, `baseline`, `matrix`, `runs`, `scenario_results`, `acceptance`, `rejections`, `recommended_next`, `final_answer`
- `runs[]` required fields:
  - `run_id`, `family`, `level`, `scenario`, `independence_ok`, `stage_funnel`, `signal_density`, `quality_proxy`, `filtered_quality`, `portfolio_metrics`, `delta_vs_baseline`, `decision`
- `stage_funnel` required:
  - integer counts for stages `0..7`
- `quality_proxy` required:
  - `pp10`, `pp20`, `rp20`
- `portfolio_metrics` required:
  - `profit_factor`, `sharpe`, `mdd_pct`, `return_pct`, `trade_count`

## 12) Decision Deliverable
- Primary output: ranked candidate settings by density-quality-risk frontier.
- Final decision statement format:
  - `ACCEPT` / `REJECT` per family level with reason and metric evidence.
- If no acceptable candidate:
  - return `NO_CHANGE_RECOMMENDED` and preserve baseline breakout setup.
