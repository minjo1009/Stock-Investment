...
Use Task604 / `docs/reports/task_604_three_day_feedback_remediation_plan/` as the current daily-feedback remediation board. It binds the latest three feedback reports to team-owned actions, blocker dependency order, and daily closeout checks.
...
## Retired GPT/Chrome Review Subagent

Task606 previously added `skills/gpt-chrome-review-subagent/SKILL.md` as a
persistent review-subagent workflow. Task3806 deletes that skill. The section
below is historical boundary context, not an active routing path.

| Lane | Lead Routing | Owner | Reviewer | Persistent Rule |
|---|---|---|---|---|
| Strategy acceptance | 필수 first | Regime Research | Research Governance | GPT may red-team acceptance language but cannot approve strategy |
| Backtest/replay | 필수 informed | Backtest & Simulation Infra | Research Governance | GPT may find missing split/OOS/leakage/cost/replay evidence but cannot certify metrics |
| Frontend/UI | 필수 informed | Frontend/UI | Research Governance | Chrome may support five-second blocker review; screenshots are human visibility evidence only |
| Data/source | 필수 informed | Data & Market Microstructure | Research Governance | GPT may review source-health checklist gaps but cannot create raw source truth |
| Execution/risk | 필수 informed | Execution & Risk | Data & Market Microstructure | GPT may review broker-truth wording but cannot verify fills |
| Slack/EOD | 필수 informed | Slack/EOD | Research Governance | GPT may improve blocker-first wording but Slack success is not trade success |
| Chart evidence | 필수 informed | Chart Evidence | Intraday Continuation Research | GPT/Chrome may review packet visibility but cannot infer lifecycle identity from images |
| Governance | 필수 informed | Research Governance | Relevant owner team | GPT findings enter backlog only through task, artifact, and validation discipline |

The deleted packet generator must not be used for new work. Historical GPT/Chrome
findings can affect repo work only after normal owner, artifact, registry, and
validation discipline converts them into repo-native evidence.

## Codex-GPT Expert Relay Skill

Task3808 adds `skills/codex-gpt-expert-relay-loop/SKILL.md` as the active
expert-prompt relay workflow for non-trivial work. Codex remains the implementer,
Chrome GPT is the external expert prompt designer or reviewer, GitHub/repo
artifacts remain SSOT, and the user remains final approver.

The relay can classify tasks, select expert roles, choose GPT mode, generate
Chrome GPT prompts, wait for returned expert prompts, guide small patches, and
generate review prompts. It cannot change strategy acceptance, deployment
readiness, broker truth, paper/live permission, or real-capital permission.
...
For daily feedback, each lead must report either `status changed` or `unchanged with explicit reason` for their blocker before the day can close. The dependency order is Task604's chain: broker-truth SELL closeout, ATR/source snapshot closure, closed candidate linkage, replay position 99%, then source, dashboard, review packet, Slack, and governance locks.
