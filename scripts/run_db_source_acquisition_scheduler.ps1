...
function Test-Due([datetime]$NowUtc, [int]$IntervalMinutes) {
    ...
    return ($minuteOfDay % $IntervalMinutes) -eq 0
}
...
while ($true) {
    ...
    $schedulerOverridePath = Join-Path $artifactDir "__no_scheduler_override__.json"
    ...
    foreach ($job in @($configObj.jobs)) {
        ...
        $secLiveAutoSchedulerEnabled = ($configObj.PSObject.Properties.Name -contains "sec_live_auto_scheduler_enabled") -and ($configObj.sec_live_auto_scheduler_enabled -eq $true)
        if ($allowNetwork -and -not $secLiveAutoSchedulerEnabled) {
            $filteredFamilies = [System.Collections.ArrayList]@()
            foreach ($family in @($families)) {
                if ([string]$family -eq "sec_events") {
                    Add-Content -Path $stdout -Value "[$stamp] SKIP sec_events live scheduler job=$safeJobName reason=SEC_LIVE_AUTO_SCHEDULER_DISABLED"
                } else {
                    [void]$filteredFamilies.Add($family)
                }
            }
        ...
        if ($families.Count -gt 0) {
            $jsonPath = Join-Path $artifactDir "$safeJobName`_run$runCount`_$($bucket.Replace(':','').Replace('-','')).json"
            $argsList = [System.Collections.ArrayList]@("-m", "tools.db.run_source_acquisition_once", "--config-path", $Config, "--override-path", $schedulerOverridePath, "--apply", "--bucket", $bucket, "--json", $jsonPath)
            if ($allowNetwork) {
        ...
        $registeredLoopEnabled = ($configObj.PSObject.Properties.Name -contains "registered_loop_enabled") -and ($configObj.registered_loop_enabled -eq $true)
        if ($registeredLoopEnabled) {
            $registeredJson = Join-Path $artifactDir "$safeJobName`_registered_loop_run$runCount`_$($bucket.Replace(':','').Replace('-','')).json"
            ...
            Add-Content -Path $stdout -Value "[$stamp] SKIP registered loop job=$safeJobName reason=REGISTERED_LOOP_DISABLED_OR_UNAVAILABLE"
        }
    }
    if ($MaxRuns -eq 1) {
        break
