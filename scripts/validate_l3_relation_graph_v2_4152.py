...
from src.brain.l3_relation_graph_v2_4152.validator import validate_artifacts


TASK_ID = "TASK-4152"
REPORT_DIR = Path("docs/reports/task_4152_l3_relation_graph_v2")


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--artifact-dir", default="data/artifacts/task_4152_l3_relation_graph_v2")
    args = parser.parse_args()
    result = validate_artifacts(args.artifact_dir)
    artifact_dir = Path(args.artifact_dir)
    artifact_dir.mkdir(parents=True, exist_ok=True)
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    (artifact_dir / "l3_relation_graph_validation.json").write_text(
