...
function Invoke-Task588Once {
    ...
    if (-not $env:TRADING_INTELLIGENCE_SIDECAR_ENABLED) { $env:TRADING_INTELLIGENCE_SIDECAR_ENABLED = "1" }
    if (-not $env:TRADING_INTELLIGENCE_SIDECAR_FETCH) { $env:TRADING_INTELLIGENCE_SIDECAR_FETCH = "1" }
    if (-not $env:TRADING_INTELLIGENCE_SIDECAR_MIN_INTERVAL_SEC) { $env:TRADING_INTELLIGENCE_SIDECAR_MIN_INTERVAL_SEC = "900" }
    if (-not $env:TRADING_INTELLIGENCE_SIDECAR_SOURCE_STORE_ONLY) { $env:TRADING_INTELLIGENCE_SIDECAR_SOURCE_STORE_ONLY = "1" }
...
Invoke-AutomationLifecycleNotice -Status "STARTED" -Detail "Paper automation started."
        ...
        if ([int]$calendar.eod_due_flag -eq 1 -and $lastEodSessionDate -ne [string]$calendar.session_date) {
            ...
            if ($eodExitCode -eq 0) {
                ...
                if (-not $lifecycleEndNoticeSent) {
                    Invoke-AutomationLifecycleNotice -Status "ENDED" -Detail "Paper automation ended."
