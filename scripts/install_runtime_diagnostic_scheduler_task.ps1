param(
    [string]$TaskName = "TraderBrainRuntimeDiagnosticScheduler",
...
$runner = Join-Path $projectRoot "scripts/run_runtime_diagnostic_scheduler.ps1"
...
$command = "Set-Location '$quotedRoot'; `$env:KIS_ENVIRONMENT='paper'; & '$quotedRunner' -Config '$quotedConfig' -IntervalSeconds $IntervalSeconds"
...
try {
    ...
    Register-ScheduledTask `
        ...
        -Trigger $trigger `
        -Settings $settings `
        -Description "Runs the Trader Brain dry-run diagnostic scheduler. Diagnostic-only; no broker submit, live order, replay, or real-capital permission." `
        -Force | Out-Null
