...
class AlpacaHistoricalBarsProvider(IntradayBarsProvider):
    ...
    def fetch_bars(
        ...
        start_ts = session_start_ts(start_date)
        end_ts = session_end_ts(end_date)
        ...
        return self._normalize_rows(symbol.upper(), rows)
    ...
    @staticmethod
    def _normalize_rows(symbol: str, rows: Iterable[dict[str, Any]]) -> pd.DataFrame:
        ...
        for row in rows:
            ...
            bar_end_dt = bar_start_dt + timedelta(minutes=5) - timedelta(seconds=1)
            normalized.append(
                {
                    "symbol": symbol,
...
def session_start_ts(day: date) -> str:
    return _utc_iso(datetime.combine(day, MARKET_OPEN, tzinfo=UTC))


def session_end_ts(day: date) -> str:
    return _utc_iso(datetime.combine(day, MARKET_CLOSE, tzinfo=UTC))
