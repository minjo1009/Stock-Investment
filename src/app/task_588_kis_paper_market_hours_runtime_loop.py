...
from .task_615_realtime_intelligence_sidecar import run_task615_realtime_intelligence_sidecar, sidecar_enabled
...
def _bucket_ts(now_iso: str, *, minutes: int) -> str:
    parsed = datetime.fromisoformat(now_iso.replace("Z", "+00:00")).astimezone(UTC)
    floored_minute = parsed.minute - (parsed.minute % minutes)
    return parsed.replace(minute=floored_minute, second=0, microsecond=0).isoformat().replace("+00:00", "Z")


def _record_safety_heartbeat_guard(db_path: Path, *, started_at: str) -> dict[str, Any]:
    bucket = _bucket_ts(started_at, minutes=5)
    state = L0L6DiagnosticRuntimeState(
        cadence=DiagnosticHeartbeatCadence.SAFETY_5_MIN,
        heartbeat_bucket_ts=bucket,
        market_session_id=f"nasdaq-{bucket[:10]}",
        market_data_asof_ts=bucket,
        ...
        thesis_ids=(),
        policy_action_ids=(),
        runtime_decision_ids=("runtime-review-state:legacy-task588",),
        order_state_refs=("orders:pre-refresh",),
        changed_candidate_ids=(),
        validation_refs=("task588_safety_heartbeat_guard_v1",),
    )
    ...
    return {
        "diagnostic_cadence": decision.cadence.value,
        "diagnostic_status": decision.status.value,
        "diagnostic_should_execute": int(decision.should_execute),
        "diagnostic_state_hash": decision.state_hash,
        "diagnostic_idempotency_key": decision.idempotency_key,
        "diagnostic_ledger_inserted": int(inserted),
        "diagnostic_reason_codes": "|".join(decision.reason_codes),
    }
...
def _iteration(*, db_path: Path, env_file: Path, symbols: str, iteration: int) -> dict[str, Any]:
    ...
    row: dict[str, Any] = {
        ...
        "finished_at_utc": "",
        "exception": "",
        "intelligence_sidecar_status": "INTELLIGENCE_SIDECAR_DISABLED",
        "intelligence_event_store_rows": 0,
        "intelligence_attached_source_lanes": 0,
        "intelligence_sidecar_trade_signal_used_flag": 0,
        "intelligence_sidecar_exception": "",
        "diagnostic_cadence": "",
        "diagnostic_status": "",
        "diagnostic_should_execute": 0,
        "diagnostic_state_hash": "",
        "diagnostic_idempotency_key": "",
        "diagnostic_ledger_inserted": 0,
        "diagnostic_reason_codes": "",
    }
    try:
        row.update(_record_safety_heartbeat_guard(db_path, started_at=started))
        if int(row["diagnostic_should_execute"]) == 0:
            row.update(
                {
                    "task583_status": "SKIPPED_DIAGNOSTIC_DUPLICATE",
                    "fresh_rows": 0,
                    "paper_order_candidate_rows": 0,
                    "task584_status": "SKIPPED_DIAGNOSTIC_DUPLICATE",
                    "runtime_decision_id": "",
                    "symbol": "",
                    "reason_code": "DUPLICATE_STATE_SKIPPED",
                    "task585_status": "SKIPPED_DIAGNOSTIC_DUPLICATE",
                    "order_status": "SKIPPED",
                    "orders_submitted": 0,
                    "broker_truth_fill_count": 0,
                    "active_broker_truth_fill_count": 0,
                    "confirmed_broker_truth_fill_total": 0,
                    "task587_status": "SKIPPED_DIAGNOSTIC_DUPLICATE",
                    "catalog_returncode": "",
                    "catalog_stdout_tail": "",
                    "catalog_stderr_tail": "",
                }
        ...
        if sidecar_enabled():
            try:
                ...
                sidecar_latest = _first(sidecar["latest_runtime_intelligence_sidecar_status.csv"])
                row.update(
                    {
                        "intelligence_sidecar_status": sidecar_latest.get("decision_status", ""),
                        "intelligence_event_store_rows": sidecar_latest.get("event_store_rows", 0),
                        "intelligence_attached_source_lanes": sidecar_latest.get("attached_source_lanes", 0),
                        "intelligence_sidecar_trade_signal_used_flag": sidecar_latest.get("sidecar_trade_signal_used_flag", 0),
                        "intelligence_sidecar_exception": sidecar_latest.get("exception", ""),
                    }
                )
            except Exception as exc:
                row["intelligence_sidecar_status"] = "INTELLIGENCE_SIDECAR_COLLECTION_ERROR"
                row["intelligence_sidecar_exception"] = str(exc)
        ...
        [
            {
                ...
                "intelligence_sidecar_status": str(latest.iloc[0].get("intelligence_sidecar_status", "")) if not latest.empty else "",
                "intelligence_event_store_rows": int(pd.to_numeric(latest.get("intelligence_event_store_rows", pd.Series([0])), errors="coerce").fillna(0).iloc[-1]) if not latest.empty else 0,
                "intelligence_sidecar_trade_signal_used_flag": int(pd.to_numeric(latest.get("intelligence_sidecar_trade_signal_used_flag", pd.Series([0])), errors="coerce").fillna(0).iloc[-1]) if not latest.empty else 0,
            }
        ]
    )
    artifacts = {
