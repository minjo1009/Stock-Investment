...
try:
    ...
    from src.brain.runtime_authority import BrokerSubmitIdempotencyPlan
    ...
    from brain.runtime_authority import BrokerSubmitIdempotencyPlan
...
def _legacy_paper_execution_allowed() -> bool:
    return str(os.environ.get("TRADING_ALLOW_LEGACY_PAPER_EXECUTION", "")).strip().lower() in {
        "1",
        "true",
        "yes",
        "y",
...
def _active_order_blocker(db_path: Path, symbol: str, *, session_reference_ts: object = None) -> str:
    ...
    session_ref = pd.to_datetime(session_reference_ts, utc=True, errors="coerce")
    if pd.isna(session_ref):
        session_ref = pd.Timestamp.now(tz=UTC)
    session_start = session_ref.to_pydatetime().replace(hour=0, minute=0, second=0, microsecond=0).timestamp()
    session_end = session_start + 24 * 60 * 60
    ...
    if not orders.empty and {"status", "symbol"}.issubset(orders.columns):
        if max_daily_orders > 0 and "submitted_at" in orders.columns:
            ...
            todays_orders = orders.loc[(submitted_all.fillna(0) >= session_start) & (submitted_all.fillna(0) < session_end)]
            if todays_orders.empty and not orders.empty:
                submitted_dt = pd.to_datetime(orders["submitted_at"], utc=True, errors="coerce")
                latest_submitted = submitted_dt.max()
                if not pd.isna(latest_submitted):
                    fallback_start = latest_submitted.to_pydatetime().replace(hour=0, minute=0, second=0, microsecond=0).timestamp()
                    fallback_end = fallback_start + 24 * 60 * 60
                    todays_orders = orders.loc[
                        (submitted_all.fillna(0) >= fallback_start) & (submitted_all.fillna(0) < fallback_end)
                    ]
...
def run_task585(*, db_path: Path, env_file: Path) -> dict[str, pd.DataFrame]:
    ...
    pre_refresh_active_blocker = ""
    if (
        decision is not None
        and str(decision.get("decision_status") or "") == "PAPER_ORDER_CANDIDATE"
        and str(os.environ.get("KIS_ENVIRONMENT", "paper")).strip().lower() == "paper"
    ):
        pre_refresh_active_blocker = _active_order_blocker(
            db_path,
            str(decision.get("symbol") or "").upper(),
            session_reference_ts=decision.get("source_price_ts") or decision.get("created_at"),
        )
    ...
    if (
        str(os.environ.get("KIS_ENVIRONMENT", "paper")).strip().lower() == "paper"
        and _legacy_paper_execution_allowed()
        and not pre_refresh_active_blocker
    ):
        try:
    ...
    if decision is None:
        ...
        active_blocker = pre_refresh_active_blocker or _active_order_blocker(
            db_path,
            symbol,
            session_reference_ts=decision.get("source_price_ts") or decision.get("created_at"),
        )
        ...
        elif not _legacy_paper_execution_allowed():
            row = skipped("LEGACY_PAPER_EXECUTION_GUARD_BLOCKED")
            _record_execution_event(db_path, row)
            execution_rows.append(row)
            failure_rows.append(row)
            active_blocker_handled = True
    ...
    if "active_blocker_handled" in locals() and active_blocker_handled:
        ...
        limit_price = float(decision.get("limit_price") or 0.0)
        decision_id = str(decision.get("decision_id") or "")
        existing_intent = get_paper_order_intent(str(db_path), idempotency_key=decision_id)
        if existing_intent is not None and str(existing_intent.get("state") or "").upper() != "CREATED":
            row = skipped("DUPLICATE_INTENT_REQUIRES_RECONCILIATION_BEFORE_RETRY")
            row["raw_response"] = {"existing_intent": existing_intent}
            _record_execution_event(db_path, row)
            execution_rows.append(row)
            failure_rows.append(row)
        else:
            broker_supports_client_order_id = False
            idempotency_plan = BrokerSubmitIdempotencyPlan(
                local_intent_id=decision_id,
                idempotency_key=decision_id,
                scheduler_lease_token=str(os.environ.get("TRADING_SCHEDULER_LEASE_TOKEN") or "legacy-task585-no-scheduler-lease"),
                broker_supports_client_order_id=broker_supports_client_order_id,
                broker_client_order_id=decision_id if broker_supports_client_order_id else "",
                reconciliation_before_retry_required=True,
            )
            create_paper_order_intent(
                str(db_path),
                intent_id=decision_id,
                idempotency_key=idempotency_plan.idempotency_key,
                runtime_decision_id=decision_id,
                authority_id=str(os.environ.get("TRADING_RUNTIME_AUTHORITY_ID") or "legacy-task585-guarded"),
                lineage_hash=str(decision.get("lineage_hash") or decision_id),
                scheduler_lease_token=idempotency_plan.scheduler_lease_token,
                symbol=symbol,
                side=side,
                quantity=quantity,
                limit_price=limit_price,
                broker_supports_client_order_id=idempotency_plan.broker_supports_client_order_id,
                broker_client_order_id=idempotency_plan.broker_client_order_id or None,
                created_at=now,
            )
        ...
        try:
            if "idempotency_plan" not in locals():
                raise RuntimeError("DUPLICATE_INTENT_REQUIRES_RECONCILIATION_BEFORE_RETRY")
            submitted_order_id = ""
            submitted_raw_response: dict[str, Any] | None = None
            local_order_recorded = False
            intent_submitting = transition_paper_order_intent(
                str(db_path),
                idempotency_key=idempotency_plan.idempotency_key,
                from_state="CREATED",
                to_state="SUBMITTING",
            ...
            if not intent_submitting:
                raise RuntimeError("PAPER_ORDER_INTENT_NOT_CREATED")
            ...
            order_id, raw_response = kis.submit_order_with_response(
                ...
                idempotency_key=idempotency_plan.idempotency_key,
                broker_client_order_id=idempotency_plan.broker_client_order_id or None,
                reconciliation_before_retry_required=idempotency_plan.reconciliation_before_retry_required,
            )
            submitted_order_id = order_id
            submitted_raw_response = raw_response
            ...
            order_id_for_row = ""
            if "submitted_order_id" in locals() and submitted_order_id:
                order_id_for_row = submitted_order_id
                order_status = "UNKNOWN"
                reason_code = "LOCAL_RECORD_FAILED_AFTER_BROKER_SUBMIT" if not local_order_recorded else "POST_SUBMIT_RECONCILIATION_FAILED"
                final_run_status = "UNKNOWN"
                if "intent_submitting" in locals() and intent_submitting and not local_order_recorded:
                    transition_paper_order_intent(
                        str(db_path),
                        idempotency_key=idempotency_plan.idempotency_key,
                        from_state="SUBMITTING",
                        to_state="UNKNOWN",
                        updated_at=utc_now(),
                        broker_order_id=submitted_order_id,
                        raw_response=submitted_raw_response or {"error": str(exc)},
                    )
            elif "intent_submitting" in locals() and intent_submitting:
                transition_paper_order_intent(
                    str(db_path),
                    idempotency_key=idempotency_plan.idempotency_key,
                    from_state="SUBMITTING",
            ...
            record_trade_run_finish(str(db_path), run_id, final_run_status, utc_now())
            row = {
                ...
                "raw_response": {"error": str(exc), "submitted_raw_response": submitted_raw_response},
                "broker_truth_fill_flag": 0,
                "filled_qty": 0.0,
                "filled_avg_price": None,
            }
